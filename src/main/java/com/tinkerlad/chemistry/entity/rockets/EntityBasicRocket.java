...
import com.tinkerlad.chemistry.utils.interfaces.IChunkLoadHandler;
import com.tinkerlad.chemistry.utils.interfaces.IMissile;
import com.tinkerlad.chemistry.utils.interfaces.ITarget;
import com.tinkerlad.chemistry.utils.vector.Vector2;
...
public class EntityBasicRocket extends Entity implements IChunkLoadHandler, IEntityAdditionalSpawnData, IMissile,
                                                         ITarget
{
	...
	public void entityInit()
	{
		...
		this.chunkLoaderInit(ForgeChunkManager.requestTicket(ICBMExplosion.instance, this.worldObj, ForgeChunkManager.Type.ENTITY));
	}
	...
	{
		if (!this.worldObj.isRemote && true && this.chunkTicket != null) // 'true' to be replaced with config for
		// chunk loading
		{
				...
				else
				{
					...
					else
					{
						...
						if (this.targetHeight > 0 && this.motionY < 0)
						{
							...
							Block blockBelow = this.worldObj.getBlock((int) this.posX, (int) this.posY - targetHeight,
									                                     (int) this.posZ);
	...
	protected void writeEntityToNBT(NBTTagCompound nbt)
	{
		...
		{
			nbt.setTag("kaiShi", this.startPos.writeToNBT(new NBTTagCompound()));
		}
		if (this.targetVector != null)
		{
			nbt.setTag("muBiao", this.targetVector.writeToNBT(new NBTTagCompound()));
		}
