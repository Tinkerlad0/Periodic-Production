...
public class EntityElementItem extends EntityItem {

	/**
	 * The age of this EntityItem (used to animate it up and down as well as expire it)
	 */
	public int age;
	public int delayBeforeCanPickup = 80;
	/**
	...
	public EntityElementItem(World par1World, double par2, double par4, double par6) {
		super(par1World);
		this.health = 5;
	...
	public EntityElementItem(World par1World, double par2, double par4, double par6, ItemStack par8ItemStack) {
	...
	public EntityElementItem(World par1World) {
		super(par1World);
		this.health = 5;
		this.hoverStart = (float) (Math.random() * Math.PI * 2.0D);
		this.setSize(0.25F, 0.25F);
	...
	public void onUpdate() {
		...
		if (this.getEntityItem() == null) {
			...
			if (this.isWet()) {
				Item item1 = this.getEntityItem().getItem();
				if (item1 instanceof ItemElementBase) {
					if (((ItemElementBase) item1).ELEMENT.TYPE == Element.Type.ALKALINE_METAL) {
						this.playSound("random.fizz", 0.4F, 2.0F + this.rand.nextFloat() * 0.4F);
						worldObj.createExplosion((Entity) null, this.posX, this.posY, this.posZ,
								                        ((ItemElementBase) item1).ELEMENT.SERIES + stack.stackSize,
								                        false);
						this.setDead();
					}
				} else if (item1 instanceof ItemElementIngot) {
					if (((ItemElementIngot) item1).ELEMENT.TYPE == Element.Type.ALKALINE_METAL) {
						this.playSound("random.fizz", 0.4F, 2.0F + this.rand.nextFloat() * 0.4F);
						worldObj.createExplosion(null, this.posX, this.posY, this.posZ,
								                        ((ItemElementIngot) item1).ELEMENT.SERIES + stack.stackSize,
								                        true);
						this.setDead();
					}
				}
	...
	public void onCollideWithPlayer(EntityPlayer par1EntityPlayer) {
		if (!this.worldObj.isRemote) {
			...
			if (this.delayBeforeCanPickup <= 0 && (this.field_145802_g == null || lifespan - this.age <= 200 || this.field_145802_g.equals(par1EntityPlayer.getCommandSenderName())) && (event.getResult() == Event.Result.ALLOW || i <= 0 || par1EntityPlayer.inventory.addItemStackToInventory(itemstack))) {
	...
	public ItemStack getEntityItem() {
		...
		if (itemstack == null) {
			if (this.worldObj != null) {
				LogHelper.error("Item entity " + this.getEntityId() + " has no item?!");
			}
