...
public class DimensionalPattern {
	...
	private DimensionalPattern(IPatternComponent... components) {
		for (IPatternComponent component : components) {
			if (component instanceof Layer) { layers.add((Layer) component); }
	 ...
	 *
	 * @param id         The id the method will either store, if it hasn't been used, or call it from the list.
	 * @param components The pattern contents
	 * @return
	 */
	public static DimensionalPattern createPattern(String id, IPatternComponent... components) {
		if (dimensionalMap.containsKey(id)) { return dimensionalMap.get(id); }
		...
		for (Layer layer : layers) {
			...
			for (Row row : layer.rows) {
				...
				for (int depth = 0; depth < row.sections.length(); depth++) {
					...
					if (type == '*') { continue; }
					if (type == ' ') {
						if (!world.isAirBlock(x + rowPos, y + layerPos, z + depth)) { return false; }
						continue;
					}
		...
		for (Layer layer : layers) {
			...
			for (Row row : layer.rows) {
				...
				for (int depth = 0; depth < row.sections.length(); depth++) {
					...
					if (type == '*') { continue; }
					if (type == ' ') {
						if (flag.isIgnoring()) { world.setBlockToAir(x + rowPos, y + layerPos, z + depth); }
						continue;
					}
					BlockState blockState = comparisonMap.get(Character.valueOf(type));
	...
	public static class Layer implements IPatternComponent {
		...
		public int[] getLengthOfRows() {
			...
			int index = 0;
			for (Row row : rows) { size[index++] = row.sections.length(); }
			return size;
		}
	}
