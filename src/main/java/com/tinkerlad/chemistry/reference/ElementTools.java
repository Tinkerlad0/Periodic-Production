...
public class ElementTools {
	...
	public void initPickaxes() {
		for (ElementToolPart toolPart : Chemistry.ELEMENT_MATERIALS.getToolPartList()) {
...
//					                            getItemForCrafting(toolPart.getCore()), 'Z', Items.stick);
	...
	public Item getItemForCrafting(Element element) {
		if (element.STATE == Element.State.SOLID) {
			return Chemistry.ELEMENT_REGISTRY.getIngotFromElement(element);
		} else {
			return Chemistry.ELEMENT_REGISTRY.getBaseItemFromElement(element);
		}
	}
